...
class Hogweed(Plant):
    def __init__(self, position, world, power=10, initiative=0, age=0):
        ...
        self._world.add_message(f"Hogweed ({self._y}, {self._x}) was created")
    ...
    def action(self):
        ...
        for cell in neighbors:
            if isinstance(cell.get_org(), Animal) and not isinstance(cell.get_org(), CyberSheep):
                    self._world.add_message(f"{self._name}: I killed {cell.get_org().get_name()} at {cell.get_position()}")
    ...
    def collision(self, org):
        ...
        if not isinstance(org, CyberSheep): # not isinstance(cell.get_org(), CyberSheep
            self._world.add_message(f"{self._name}: Organism ({org.get_name()}) ate me, and I killed it")
            self._world.delete_organism(org)
        else:
            self._world.add_message(f"{self._name}: Organism ({org.get_name()}) ate me, and I didn't kill it")